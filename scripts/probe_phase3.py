...
async def main():
    ...
    await probe_selectors(
        page,
        "Speculative selectors (from chatgpt.py)",
        [
            '[data-testid*="temp"]',
    ...
    for item in temp_scan or []:
        print(
            f"    <{item['tag']}> testid={item['testId']} label={item['ariaLabel']} @ {item['rect']}"
        )
        if item["text"]:
    ...
    toolbar_scan = await _cdp_run_js(
        page,
        """(() => {
        const results = [];
    ...
    for item in toolbar_scan or []:
        print(
            f'    <BUTTON> testid={item["testId"]} label="{item["ariaLabel"]}" pressed={item["ariaPressed"]} @ {item["rect"]}'
        )
        if item["text"]:
            print(f'        text="{item["text"]}"')
    ...
    input_area_buttons = await _cdp_run_js(
        page,
        """(() => {
    ...
    if not first_chat:
        ...
        print(
            f'  First chat: "{first_chat["text"]}" at ({first_chat["x"]:.0f},{first_chat["y"]:.0f})'
        )
        ...
        hover_buttons = await _cdp_run_js(
            page,
            f"""(() => {{
            const results = [];
            const btns = document.querySelectorAll('button');
        ...
        if hover_buttons:
            ...
            for item in menu_items or []:
                print(
                    f'    [{item["rect"]}] role={item["role"]} testid={item["testId"]} text="{item["text"]}"'
                )

            if not menu_items:
                # Fallback: check for any popup/dialog
                popup = await _cdp_run_js(
                    page,
                    """(() => {
